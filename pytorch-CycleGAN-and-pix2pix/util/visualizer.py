...
class Visualizer:
    ...
    def __init__(self, opt):
        ...
        if self.use_wandb:
            ...
            if not dist.is_initialized() or dist.get_rank() == 0:
                self.wandb_project_name = getattr(
                    opt, "wandb_project_name", "CycleGAN-and-pix2pix"
                )
                self.wandb_run = (
                    wandb.init(
                        project=self.wandb_project_name, name=opt.name, config=opt
                    )
                    if not wandb.run
                    else wandb.run
                )
    ...
    def display_current_results(
        self, visuals, epoch: int, total_iters: int, save_result=False
    ):
        ...
        if self.use_wandb:
            ...
            for label, image in visuals.items():
                ...
                wandb_image = wandb.Image(
                    image_numpy, caption=f"{label} - Step {total_iters}"
                )
        ...
        if self.use_html and (
            ...
            # update website
            webpage = html.HTML(
                self.web_dir, f"Experiment name = {self.name}", refresh=1
            )
