...
class TestBeforePush:
    ...
    def setup_datasets(self):
        ...
        if not Path("./datasets/mini").exists():
            subprocess.run(
                ["bash", "./datasets/download_cyclegan_dataset.sh", "mini"], check=True
            )

        if not Path("./datasets/mini_pix2pix").exists():
            subprocess.run(
                ["bash", "./datasets/download_cyclegan_dataset.sh", "mini_pix2pix"],
                check=True,
            )
    ...
    def test_cyclegan_train_test(self):
        ...
        ), f"CycleGAN training failed: {train_result.stderr}"
        ...
        ), f"CycleGAN testing failed: {test_result.stderr}"
    ...
    def test_pix2pix_train_test(self):
        ...
        assert (
            train_result.returncode == 0
        ), f"Pix2pix training failed: {train_result.stderr}"
        ...
        assert (
            test_result.returncode == 0
        ), f"Pix2pix testing failed: {test_result.stderr}"
    ...
    def test_template_train_test(self):
        ...
        assert (
            train_result.returncode == 0
        ), f"Template training failed: {train_result.stderr}"
        ...
        assert (
            test_result.returncode == 0
        ), f"Template testing failed: {test_result.stderr}"
    ...
    def test_colorization_train_test(self):
        ...
        assert (
            train_result.returncode == 0
        ), f"Colorization training failed: {train_result.stderr}"
        ...
        assert (
            test_result.returncode == 0
        ), f"Colorization testing failed: {test_result.stderr}"
