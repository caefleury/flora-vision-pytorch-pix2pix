...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        self.save_dir = (
            Path(opt.checkpoints_dir) / opt.name
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if not self.isTrain or opt.continue_train:
                    load_suffix = (
                        f"iter_{opt.load_iter}" if opt.load_iter > 0 else opt.epoch
                    )
                ...
                if dist.is_initialized():
                    ...
                    if self.opt.norm == "syncbatch":
                        raise ValueError(
                            f"For distributed training, opt.norm must be 'syncbatch' or 'inst', but got '{self.opt.norm}'. "
                            "Please set --norm syncbatch for multi-GPU training."
                        )
            ...
            if isinstance(name, str):
                ...
                if torch.cuda.is_available():
                    if "LOCAL_RANK" in os.environ:
                        ...
                        net.to(local_rank)
                        print(
                            f"Initialized network {name} with device cuda:{local_rank}"
                        )
                    else:
                        net.to(0)
